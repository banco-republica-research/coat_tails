...
current_first_des <- cbind(t(table(l$win_t)),t(table(l2$win_t)),t(table(l1$win_t)))
current_first_des
...
nocurrent_first_des <- cbind(t(table(l$win_t)),t(table(l2$win_t)),t(table(l1$win_t)))
nocurrent_first_des
...
l_f <- function(o){
  r <- rdrobust(y = l[,o],
                ...
                all = T,
                vce = "nn")
  pdf(str_c(results, "/Graphs/First_round", "/RD_presfirst_nocurrent.pdf"), height=6, width=12)
  rdplot(y=l2[,o], x=l2$margin_prop_2, c = 0,
         # y.lim = c(0.2, 0.8),
         # x.lim = c(0.45, 0.55),
         title = " ",
...
r <- lapply(out, l_f)
saveRDS(r, str_c(results, "/coat_tails_presfirst_nocurrent1_coalition.rds"))
r
...
des <- rbind(first_des, current_first_des,nocurrent_first_des, party_des)
des
