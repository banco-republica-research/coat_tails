...
presidentes <- lapply(list_files, read_dta) 

#Make all names the same (the 1998 data has english name variables)

names(presidentes[[1]]) <- names(presidentes[[2]])
...
presidentes_aggregate <- presidentes %>%
  lapply(., function(x){
...
presidentes_merge <- presidentes_aggregate %>%
  ldply() %>%
...
saveRDS(presidentes_merge,paste0(res,"presidentes_merge.rds"))
