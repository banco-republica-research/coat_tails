...
list_files <- list.files("Placebos", full.names = T) %>% .[c(1,3,4,5)]
...
sens_df_elec <- mapply(rd_to_df, list = sens_test, 
                  name = list("House", "Presidential (Final-Round)", "Senate", "Mayor"), 
                  SIMPLIFY = F) %>%
  ldply() %>% 
  ...
  subset(Bandwidth <= 0.25 & Bandwidth >= 0.05) %>%
  mutate(.id = factor(.$.id, levels = c('Mayor', 'House', 'Senate', 'Presidential (Final-Round)')))
...
g <- g + coord_cartesian(xlim = c(0.05, 0.25))
...
g <- g + theme_bw()
g
ggsave(str_c(doc,"RDggplot_sens_test.pdf"), width=30, height=20, units="cm")
...
g <- g + theme_bw()
g
ggsave(str_c(doc,"RDggplot_sens_test_inv.pdf"), width=30, height=20, units="cm")
